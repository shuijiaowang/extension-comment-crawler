...
const parent_comment_content=parent_comment_body.querySelector(":scope > .CommentContent").textContent  //评论内容
const parent_comment_picture=parent_comment_body.querySelector(":scope > .CommentContent  .comment_img")?"[图片]":""  //图片
...
//一级评论页的滚动
document.querySelector(".Modal-content > div > div:nth-child(2)").scrollBy(0,100)//.scrollTop,滚动和判断是否到底。

const reply_comment_show_more_button=parent_comment_item.querySelector(":scope > button") //.click()查看一级评论下的全部回复,会打开一个新的悬浮层
...
const reply_comment_content=reply_comment_body.querySelector(":scope > .CommentContent").textContent  //评论内容
...
//二级评论页的滚动
const return_parent_button=document.querySelector(".Modal-content > div:nth-child(2) > div:nth-child(1) span")//.click()回到一级评论页。
document.querySelector(".Modal-content > div:nth-child(2) > div:nth-child(2)").scrollBy(0,100) //.scrollTop,滚动和判断是否到底。
